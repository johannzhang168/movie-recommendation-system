        ...
        for epoch_count in range(self.n_epochs): 
            ...
            for i, batch_tuple in enumerate(batch_loader):
                ...
                if do_report_now:
                    ...
                    print("epoch %11.3f | loss_total % 11.5f | train_AUC% 11.5f | valid_AUC % 11.5f | %s | train_MAE% 11.5f | valid_MAE% 11.5f" % (
                        epoch, loss if epoch <= 2 else smooth_loss,
                        train_perf_dict['auc'], valid_perf_dict['auc'],
                        avg_grad_norm_str, train_perf_dict['mae'], valid_perf_dict['mae']))
